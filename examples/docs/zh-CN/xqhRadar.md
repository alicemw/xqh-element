    ...
    series: [
      {
        ...
        borderWidth: '0px', // 非必传, 用于图案边框宽度，可传数字，类似1 2 3 4 5
        full: true, // 非必传，用于控制当前图形是否填满颜色 可传true || false, 默认false
        linearGradient: ['#1051B5', '#2F76E3'], // 非必传, 用于背景渐变色，目前只支持从左到右渐变，如不传，背景颜色按照color展示
        color: '#1051B5', // 必传，用于底部对应文案，数据点颜色展示, 可传 css颜色代码
        opacity: .8 //  非必传， 用于背景透明度展示，默认为1，可传小数 .1 .2 .3
      },
      {
        name: '行业均值',
        data: [
